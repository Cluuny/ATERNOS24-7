    ...
    if (blockEntity.id === 'minecraft:sign' || blockEntity.id === 'Sign') {
      const prepareJson = (i) => {
        ...
        try {
          ...
          console.error(`Error al parsear JSON para Text${i}:`, e);
          console.error(`Contenido de Text${i}:`, data);
          return '';
        }

        // Si json es una cadena, envolverla en un objeto con la propiedad 'text'
        if (typeof json === 'string') {
          json = { text: json };
        }

        // Verificar si 'json' ahora es un objeto con la propiedad 'text'
        if (typeof json === 'object' && 'text' in json) {
          // Asegurarse de que 'text' es una cadena antes de reemplazar
          if (typeof json.text === 'string') {
            json.text = json.text.replace(/^"|"$/g, '');
          } else {
            console.warn(`La propiedad 'text' en Text${i} no es una cadena.`);
          }
          return json;
        }

        // Si no cumple con las condiciones anteriores, retornar una cadena vacía
        return '';
      }
